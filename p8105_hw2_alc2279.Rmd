...
subway_data %>% 
  filter(ada == TRUE) %>% 
  nrow(.)
## needs to be a subset of the first 
```
...
##TAKE MEAN OF LOGICAL VARIABLE TO GET PROPORTION

#find total number of entrances/exits
all_entrances_exits = nrow(subway_data)

#find proportion -- homework 1; tried functions on different variables
entry_no_vending / all_entrances_exits

##den - stations with no vending
##num - allow entry of those
##need to take the mean of a filtered
```
...
##Question - can I just use the range? or do i need to use cell_cols and omit? can use it, can also use cell_colls, just explain why you're doing it
